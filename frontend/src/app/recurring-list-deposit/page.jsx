...
const RecurringListDeposit = () => {

    const [depositData, setDepositData] = useState([]);
    const [currentUser, setCurrentUser] = useState(JSON.parse(localStorage.getItem('user')));
    const fetchData = async () => {
        const res = await axios.get('http://localhost:5000/recurring/getbyemail/'+currentUser?.email)
        const data = res.data;
        console.table(data);
        setDepositData(data);
    }
...

                                        {
                                            depositData.map(deposit => (
                                                <tr key={deposit} className="bg-white hover:bg-gray-50">
                                                    ...
                                                    <td className="size-px whitespace-nowrap">
                                                        <Link
                                                        href={'/deposit-details-recurring/' + deposit._id}
                                                            type="button"
                                                            className="block"
                                                            aria-haspopup="dialog"
                                                            aria-expanded="false"
...
export default RecurringListDeposit;